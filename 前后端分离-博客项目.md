...
例如：hmac.new(b'1234',b'6666',digestmod='SHA256')
#十六进制结果  'a759fb2b3a20f4aa86ac0ea5b8f5f34e5c26263d057eef5a306017ed6b45cd9b'
h.digest() #获取最终结果
h.hexdigest() #以十六进制获取最终结果（加hex表示）
接收到加密信息的人会用相同的加密函数将 内容 和 key 进行比对，如果比对的结果和发送者加密后的值相同就能认定 发送者是谁
```
...
​        前两个base64算法可以解密数据，但是加上key之后用的hmac算法是不可逆的，因此只有输入正确的key才能解出正确的签名
...
issuer字段不影响key的校验，即使在pyload中添加了该字段，用key依旧可以解出，只有在解时也添加了这个字段才会触发该字段的进一步校验
...
​				如果请求头中的Origin在服务器接受范围内(即接受的域在服务器白名单中)， 则返回如下头

| 响应头                           | 作用                                                         | 备注 |
| -------------------------------- | ------------------------------------------------------------ | ---- |
| Access-Control-Allow-Origin      | 服务器接受得域(重点)                                         |      |
...
​		如果服务器不接受此域，则响应头中不包含 Access-Control-Allow-Origin，若包含，则返回数据

## 5，预检请求发送流程

​	1，OPTION 请求发起，携带如下请求头(与post，get一样时request.method方法)

| 请求头                         | 作用                 | 备注 |
| ------------------------------ | -------------------- | ---- |
| Origin                         | 表明此请求来自哪个域 | 必选 |
| Access-Control-Request-Method  | 此次请求使用方法(post,get,head等)     必选 |
| Access-Control-Request-Headers | 此次请求使用的头(自定义请求头)     | 必选 |
...
```python
		...
		3，CORS_ORIGIN_ALLOW_ALL = 布尔值  如果为True 白名单不启用(所有网站都可以进行跨域访问)
		...
		7, CORS_PREFLIGHT_MAX_AGE  默认 86400s(时限)  #在须要多次请求(比如预检请求)时，浏览器会将第一次请求缓存，在第一次之后就只用发送一次主请求就好
		8, CORS_EXPOSE_HEADERS  []  #服务端自定义头
